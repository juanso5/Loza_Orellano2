...
export function AppDataProvider({ children }) {
  ...
  const refreshClientes = useCallback(async () => {
    ...
    try {
      ...
      console.log('[AppDataProvider] Clientes cargados:', list.length);
    } catch (err) {
      console.error('[AppDataProvider] Error cargando clientes:', err);
  ...
  const refreshFondos = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(json?.data) ? json.data : [];
      setFondos(list);
      console.log('[AppDataProvider] Fondos cargados:', list.length);
    } catch (err) {
      console.error('[AppDataProvider] Error cargando fondos:', err);
      setFondos([]);
    } finally {
  ...
  const refreshLiquidez = useCallback(async () => {
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        console.log('[AppDataProvider] Liquidez cargada:', json.data);
      } else {
        setLiquidez(null);
      }
    } catch (err) {
      console.error('[AppDataProvider] Error cargando liquidez:', err);
      setLiquidez(null);
    } finally {
  ...
  const refreshMovimientos = useCallback(async (limit = 10000) => {
    ...
    try {
      ...
      const list = Array.isArray(json?.data) ? json.data : [];
      setMovimientos(list);
      console.log('[AppDataProvider] Movimientos cargados:', list.length);
    } catch (err) {
      console.error('[AppDataProvider] Error cargando movimientos:', err);
      setMovimientos([]);
    } finally {
  ...
  const refreshAll = useCallback(async () => {
    ...
    console.log('[AppDataProvider] Refrescando todos los datos...');
    await Promise.all([
      refreshFondos(),
      refreshLiquidez(),
      refreshMovimientos(),
